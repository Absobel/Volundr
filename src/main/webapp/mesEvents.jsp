            ...
            for (Groupe g : gs){
                if (!(g.getIsNotEventGroup())){
                    ...
                    for( Evenement ev : g.getEvenements()){
                        ...
                        <%= s %><br><br>
                        <%
                        if (ev.getDebutInscr() != null && ev.getFinInscr() != null){
                            LocalDateTime debut = ev.getDebutInscr().toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime();
                            LocalDateTime fin = ev.getFinInscr().toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime();
                        
                            if(LocalDateTime.now().isBefore(debut)){
                                %>
                                L'inscription n'a pas commencée. Début de l'inscription : <%= dateFormat.format(ev.getDebutInscr()) %>
                                <%
                            }
                            else if (debut.isBefore(LocalDateTime.now()) &&  LocalDateTime.now().isBefore(fin)) { %>
